#include "vision/pos_transformer_real.hpp"
...
Pos_Transformer::Pos_Transformer()
{
    ...
    int i = 0;
    tf_listener.waitForTransform("/odom","/ardrone_base_bottomcam", ros::Time(0), ros::Duration(3.0));         
    try{
        tf_listener.lookupTransform("/odom","/ardrone_base_bottomcam",ros::Time(0), transform);
...
void  Pos_Transformer::transformation(const geometry_msgs::PoseStamped &centroid){
    ...
    tf_listener.waitForTransform("/odom","/ardrone_base_bottomcam", ros::Time(0), ros::Duration(0.5)); //0.05
    tf_listener.lookupTransform("/odom","/ardrone_base_bottomcam",ros::Time(0), transform);
    ...
    estimated_centroid_in_world.header.frame_id = "/odom";
