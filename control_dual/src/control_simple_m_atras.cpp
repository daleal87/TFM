...
std_msgs::Int32 accion_ard2_msg, accion_p3at_msg, longitud_objetivo_msg, joy_cable_vel_msg;
...
std::string ard2_pose_topic_name, p3at_pose_topic_name, sonar_topic_name;
...
int long_cable_encoder = 0;
float longitud_objetivo_float = 0.0;
...
bool fin_mision, fin_despegue, fin_aterrizaje, aproximacion_prima, aproximacion_final, imu_activa;
...
double long_cable_max, long_cable_min, long_cable_inic;
bool activar_parada_longitud, activar_deteccion_obstaculos, drone_a_pioneer_aterrizando;
int n_ptos_esquiva;
...
{
    orientacion_cable = M_PI + orientacion_cable_msg->data; // Leemos el valor del topic que indica la orientacion del cable y lo "invertimos" (que este alineado con el -eje_x del Pioneer)
    if (orientacion_cable > M_PI ) { orientacion_cable -= 2 * M_PI; } // Acotamos el valor entre -pi y pi
    // m_atras
}
...
void distanrobotsCallback(const std_msgs::Float64::ConstPtr& msg)
{
    distancia_entre_robots = msg->data; // Leemos el valor del topic que indica la longitud del cable
}

void longitudcableCallback(const std_msgs::Int32::ConstPtr& msg)
{
...
void ard2_pose_topic_Callback(const nav_msgs::Odometry::ConstPtr& ard2_kalman_odom_msg)
{
...
void sonar_topic_Callback(const std_msgs::Int32::ConstPtr& msg)
{
    h_kalman_sonar = msg->data; // Altura segun el filtro de kalman con solo el sonar

    if (fin_despegue == false && !modo_automatico && h_kalman_sonar > altura_vuelo*0.5*1000)
    {
        fin_despegue = true;
    }
}

void p3at_pose_topic_Callback(const nav_msgs::Odometry::ConstPtr& p3at_kalman_odom_msg)
{
...
{   
    if (modo_automatico && msg->data == false) // Cancelacion del modo automatico (y del aterrizaje)
    {
...
void modoaterrizajeCallback(const std_msgs::Bool::ConstPtr& msg)
{
    if (accion_ard2 != ARD2_ATERRIZANDO && msg->data == true)
    {
        ...
        ROS_INFO("----------------------------");
        ROS_INFO("MODO ATERRIZAJE ACTIVADO");
        ROS_INFO("----------------------------");
    }
}
...
{    
    ...
    {
        teleop_cable = true;
        longitud_objetivo_float += vel_msg->data;
        longitud_objetivo_msg.data = (int)longitud_objetivo_float;
        if (longitud_objetivo_msg.data < 200) {longitud_objetivo_msg.data = 200;}
    }
}
...
void imuactivaCallback(const std_msgs::Bool::ConstPtr& msg)
{
    if (imu_activa == false && msg->data == true)
    {
        imu_activa = true;
    }
}
...
double control_velocidad_lineal(double diferencia, double pre_error, double kp, double ki, double kd, double vmax)
{
...
double control_velocidad_angular(double diferencia, double kp, double vmax)
{
...
/* double control_velocidad_angular_p3at(double diferencia, double kp, double vmax) // m_atras
{
    double vel = kp * diferencia;

    if (vel > 0 && vel > 0.25)
    {
        if ( (2*M_PI-diferencia)/vmax < diferencia/0.25 )
        {
            vel = -vmax;
        }
        else
...
geometry_msgs::Twist limitar_velocidad(geometry_msgs::Twist joy_vel_msg, double vmax_linear, double vmax_angular)
{
    geometry_msgs::Twist vel_limit;

    if (joy_vel_msg.linear.x > 0 && joy_vel_msg.linear.x > vmax_linear) { vel_limit.linear.x = vmax_linear; }
    else if (joy_vel_msg.linear.x < 0  && joy_vel_msg.linear.x < -vmax_linear) { vel_limit.linear.x = -vmax_linear; }
    else { vel_limit.linear.x = joy_vel_msg.linear.x; }

    if (joy_vel_msg.linear.y > 0 && joy_vel_msg.linear.y > vmax_linear) { vel_limit.linear.y = vmax_linear; }
    else if (joy_vel_msg.linear.y < 0  && joy_vel_msg.linear.y < -vmax_linear) { vel_limit.linear.y = -vmax_linear; }
    else { vel_limit.linear.y = joy_vel_msg.linear.y; }

    /*if (joy_vel_msg.linear.z > 0 && joy_vel_msg.linear.z > vmax_linear) { vel_limit.linear.z = vmax_linear; }
    else if (joy_vel_msg.linear.z < 0  && joy_vel_msg.linear.z < -vmax_linear) { vel_limit.linear.z = -vmax_linear; }
    else { vel_limit.linear.z = joy_vel_msg.linear.z; } */

    vel_limit.linear.z = joy_vel_msg.linear.z;

    /*if (joy_vel_msg.angular.x > 0 && joy_vel_msg.angular.x > vmax_angular) { vel_limit.angular.x = vmax_angular; }
    else if (joy_vel_msg.angular.x < 0  && joy_vel_msg.angular.x < -vmax_angular) { vel_limit.angular.x = -vmax_angular; }
    else { vel_limit.angular.x = joy_vel_msg.angular.x; }*/

    /*if (joy_vel_msg.angular.y > 0 && joy_vel_msg.angular.y > vmax_angular) { vel_limit.angular.y = vmax_angular; }
    else if (joy_vel_msg.angular.y < 0  && joy_vel_msg.angular.y < -vmax_angular) { vel_limit.angular.y = -vmax_angular; }
    else { vel_limit.angular.y = joy_vel_msg.angular.y; }*/

    //vel_limit.angular.x = 0.0;
    //vel_limit.angular.y = 0.0;

    if (joy_vel_msg.angular.z > 0 && joy_vel_msg.angular.z > vmax_angular) { vel_limit.angular.z = vmax_angular; }
    else if (joy_vel_msg.angular.z < 0  && joy_vel_msg.angular.z < -vmax_angular) { vel_limit.angular.z = -vmax_angular; }
    else { vel_limit.angular.z = joy_vel_msg.angular.z; }

    return vel_limit;
}

geometry_msgs::Twist limitar_velocidad_p3at(geometry_msgs::Twist joy_vel_msg, double vmax_linear, double vmax_angular) //m_atras
{
    geometry_msgs::Twist vel_limit;

    if (joy_vel_msg.linear.x > 0) { vel_limit.linear.x = 0.0; }
    else if (joy_vel_msg.linear.x < 0  && joy_vel_msg.linear.x < -vmax_linear) { vel_limit.linear.x = -vmax_linear; }
    else { vel_limit.linear.x = joy_vel_msg.linear.x; }

    vel_limit.linear.y = 0.0;
    vel_limit.linear.z = 0.0;
    //vel_limit.angular.x = 0.0;
    //vel_limit.angular.y = 0.0;
...
std::tuple<bool, double> detector_obstaculos(double h_kalman_odom, double h_kalman_sonar_b, unsigned int accion_ard2)
{   
    if (h_kalman_odom * 0.6 > h_kalman_sonar_b && h_kalman_odom - h_kalman_sonar_b > 0.3 && accion_ard2 >= 2) {
        obstaculo_detectado = true;
        h_obstaculo = h_kalman_odom - h_kalman_sonar_b;
    }
    else {
...
geometry_msgs::Twist control_ard2(geometry_msgs::Pose p_ard2, geometry_msgs::Pose p_obj_ard2)
{
    ...
    vel_msg.linear.x = control_velocidad_lineal(diff.getOrigin().x(), pre_ard2_error_x, kp_ard2, kd_ard2, ki_ard2,  ard2_vmax_linear);
    vel_msg.linear.y = control_velocidad_lineal(diff.getOrigin().y(), pre_ard2_error_y, kp_ard2, kd_ard2, ki_ard2,  ard2_vmax_linear);
    vel_msg.linear.z = control_velocidad_lineal(diff.getOrigin().z(), pre_ard2_error_z, kp_ard2, kd_ard2, ki_ard2,  ard2_vmax_linear);

    //vel_msg.angular.x = 0.0;
    //vel_msg.angular.y = 0.0;
    vel_msg.angular.z = control_velocidad_angular(tf::getYaw(diff.getRotation()), 10*kp_ard2, ard2_vmax_angular);
...
{
    geometry_msgs::Twist vel_msg;
    // double diff_orientaciones;
    // if ( desvio_orient_obj >= 0 ) { diff_orientaciones = desvio_orient_obj - M_PI; }
    // else { diff_orientaciones = desvio_orient_obj + M_PI ; }

    if (accion == P3AT_SEGUIMIENTO)
    {
        // Calculamos cmd_vel
        if (distancia_pto_obj > distancia_a_ard2 && std::abs(desvio_orient_obj) > M_PI/3 )
        // if (distancia_pto_obj > distancia_a_ard2 && std::abs(diff_orientaciones) < M_PI/4 )
        {
            vel_msg.angular.z = desvio_orient_obj;
            if (vel_msg.angular.z > 0 && vel_msg.angular.z > 0.25) // Las ruedas derechas se bloquean con un vel_angular > 0.25, independientemente de la vel_lineal que se añada
            {
                if ( (2*M_PI-desvio_orient_obj)/p3at_vmax_angular < desvio_orient_obj/0.25 ) { vel_msg.angular.z = -p3at_vmax_angular; }
                else { vel_msg.angular.z = 0.25; }
            }
            else if (vel_msg.angular.z < 0  && vel_msg.angular.z < -p3at_vmax_angular) { vel_msg.angular.z = -p3at_vmax_angular; }
        }
        else if ( distancia_pto_obj > distancia_a_ard2 && std::abs(desvio_orient_obj) <= M_PI/3 )
        {
            vel_msg.linear.x = - control_velocidad_lineal(distancia_pto_obj, pre_p3at_error_x, kp_p3at, kd_p3at, ki_p3at,  p3at_vmax_linear); // m_atras

            vel_msg.angular.z = desvio_orient_obj;
            if (vel_msg.angular.z > 0 && vel_msg.angular.z > 0.25) { vel_msg.angular.z = 0.25; }
            else if (vel_msg.angular.z < 0  && vel_msg.angular.z < -p3at_vmax_angular) { vel_msg.angular.z = -p3at_vmax_angular; }
        }
        else {
...
geometry_msgs::Twist control_p3at(geometry_msgs::Pose p_p3at, geometry_msgs::Pose p_obj_p3at)
{
    ...
    if (std::abs(desviacion_orientacion) < 0.2 ) {
        vel_msg.linear.x = - control_velocidad_lineal(distancia_objetivo, pre_p3at_error_x, kp_p3at, kd_p3at, ki_p3at,  p3at_vmax_linear);
    ...
    //vel_msg.angular.z = control_velocidad_angular(desviacion_orientacion, 10*kp, p3at_vmax_angular);

    return vel_msg;
}

geometry_msgs::Twist control_mision_p3at(geometry_msgs::Pose p_p3at, geometry_msgs::Pose p_obj_p3at) // m_atras
{
    geometry_msgs::Twist vel_msg;
    vel_msg.linear.x = vel_msg.linear.y = vel_msg.linear.z = 0.0;
    vel_msg.angular.x = vel_msg.angular.y = vel_msg.angular.z = 0.0;
    ...
    //double desviacion_orientacion = atan2(diff.getOrigin().y(), diff.getOrigin().x()); // m_atras

    double desviacion_orientacion = atan2(-diff.getOrigin().y(), -diff.getOrigin().x()); // m_atras

    //double desviacion_orientacion = M_PI - atan2(diff.getOrigin().y(), diff.getOrigin().x()); // m_atras
    //if (desviacion_orientacion > M_PI) { desviacion_orientacion -= 2 * M_PI; }
...
geometry_msgs::Twist orientar_p3at(geometry_msgs::Pose p_p3at, geometry_msgs::Pose p_obj_p3at)
{
    ...
    // vel_msg.angular.z = control_velocidad_angular(tf::getYaw(diff.getRotation()), 10*kp, p3at_vmax_angular);
...
int main(int argc, char **argv)
{
    ...
    // Control cable
    nh_param.param<double>("k_extension_cable", k_extension_cable, 1.2);
    ...
    nh_param.param<double>("long_cable_max", long_cable_max, 5.0);
    nh_param.param<double>("long_cable_min", long_cable_min, 1.0);
    nh_param.param<double>("long_cable_inic", long_cable_inic, 2.0);
    ...
    nh_param.param<bool>("drone_a_pioneer_aterrizando", drone_a_pioneer_aterrizando, true);
    ...
    // Nombre topics
    nh_param.param<std::string>("ard2_pose_topic_name", ard2_pose_topic_name, "/ardrone/odometry");
    nh_param.param<std::string>("sonar_topic_name", sonar_topic_name, "/ardrone/navdata_altitude");
    nh_param.param<std::string>("p3at_pose_topic_name", p3at_pose_topic_name, "/rosaria/pose");
    ...
    ROS_INFO("Longitud minima del cable (salvo aterrizaje): %2f m", long_cable_min);
    ROS_INFO("Longitud inicial del cable (hasta fin de despegue): %2f m", long_cable_inic);
    ...
    ROS_INFO("Drone se dirige hacia el Pioneer en el aterrizaje?: %d", drone_a_pioneer_aterrizando);

    ROS_INFO("--- PARAMETROS MISION ---");
    ROS_INFO("Objetivo mision --> X: %2f, Y: %2f", misionx, misiony);

    ROS_INFO("--- NOMBRE TOPICS ---");
    ROS_INFO("Topic posicion ARD2 = %s", ard2_pose_topic_name.c_str());
    ROS_INFO("Topic sonar ARD2 = %s", sonar_topic_name.c_str());
    ROS_INFO("Topic posicion P3AT = %s", p3at_pose_topic_name.c_str());



    // Subcribers
    ros::Subscriber proyeccionXY_cable_subs = node.subscribe("cable/proyeccionXY", 1, proyeccionXYCallback);
    ros::Subscriber orientacion_cable_subs = node.subscribe("cable/orientacion", 1, orientacionCallback);
    ros::Subscriber inclinacion_cable_subs = node.subscribe("/cable/inclinacion", 1, inclinacionCallback);
    ros::Subscriber distancia_entre_robots_subs = node.subscribe("/cable/distancia_robots", 1, distanrobotsCallback);
    ros::Subscriber imu_activa_subs = node.subscribe("/cable/imu_activa", 1, imuactivaCallback);
    ros::Subscriber longitud_cable_subs = node.subscribe("/cable/longitud", 1, longitudcableCallback);
    ros::Subscriber kalman_ard2_odom_subs = node.subscribe(ard2_pose_topic_name, 1, ard2_pose_topic_Callback);
    ros::Subscriber kalman_sonar_subs = node.subscribe(sonar_topic_name, 1, sonar_topic_Callback);
    ros::Subscriber kalman_p3at_odom_subs = node.subscribe(p3at_pose_topic_name, 1, p3at_pose_topic_Callback);
    ...
    ros::Publisher longitud_cable_pub = node.advertise<std_msgs::Int32>("guiado/longitud_objetivo", 1);
    ...
    while (node.ok()){
        ...
        // CONDICION 6 || __OK__
        else if (accion_ard2 == ARD2_VUELOSOBREPLATAFORMA && accion_p3at == P3AT_MISIONFINALIZADA)
        {
            accion_ard2 = ARD2_VUELOSOBREPLATAFORMA;
            accion_p3at = P3AT_SEGUIMIENTO;
        ...
        else if (accion_ard2 == ARD2_ATERRIZANDO)
        {
            ...
            if (!aproximacion_prima && !aproximacion_final)
            {
                ...
                if (h_kalman_sonar >= altura_vuelo * 1000 * 0.7)
                {
                ...
                // Velocidad del ARD2
                if (drone_a_pioneer_aterrizando)
                {
            ...
            else if (aproximacion_prima && !aproximacion_final)
            {
                ...
                //std::tie(d_objetivo_ard2, d_altura_ard2) = evaluar_distancias(pose_ard2, objetivo_aterrizaje);
                // if (d_objetivo_ard2 < 0.3 && d_altura_ard2 < 0.2)
                if (long_cable_encoder < 500 * 1.2)
                {
                    aproximacion_final = true;
                    ROS_INFO("APROXIMACION FINAL AL PIONEER");
        ...
        else
        {
            ...
            {
                ard2_vel_msg = limitar_velocidad(joy_ard2_vel_msg, ard2_vmax_linear, ard2_vmax_angular);
            }
            else
            {
                ard2_vel_msg = parar_robot();
        ...
        {
            p3at_vel_msg = control_mision_p3at(pose_p3at, objetivo_mision);

            std::tie(d_objetivo_p3at, d_altura_p3at) = evaluar_distancias(pose_p3at, objetivo_mision);
            //rot_objetivo_p3at = std::abs(desviacion_yaw(pose_p3at, objetivo_mision)); // m_atras
            //rot_objetivo_p3at = M_PI - std::abs(desviacion_yaw(pose_p3at, pose_obj_p3at)); // m_atras
            //if (rot_objetivo_p3at > M_PI) { rot_objetivo_p3at -= 2 * M_PI; }

            //ROS_INFO("------------------");
            //ROS_INFO("v_x: %2f - v_rot_z: %2f", p3at_vel_msg.linear.x, p3at_vel_msg.angular.z);
            //ROS_INFO("d_obj: %2f - rot_obj: %2f",d_objetivo_p3at, p3at_vel_msg.angular.z);

            if (d_objetivo_p3at < 0.3 && std::abs(p3at_vel_msg.angular.z) < 0.2)
            {
        ...
        else
        {
            if (teleop_p3at == true)
            {
                p3at_vel_msg = limitar_velocidad_p3at(joy_p3at_vel_msg, p3at_vmax_linear, p3at_vmax_angular); //m_atras
            }
            else
            {
        ...
        // m_atras
        // Correcion de velocidades para la marcha atras cuando hay rotaciones hacia la izquierda
        // Se evita que las ruedas derechas se bloqueen añadiendo la componente lineal a la velocidad del robot
        // De esta manera, el Pioneer no rotara sobre si mismo, sino que avanzara girando hacia la izquierda
        // Si puede rotar hacia la derecha en parado
        if ( p3at_vel_msg.linear.x > 0.0 ) { p3at_vel_msg.linear.x = 0.0; }
        if ( p3at_vel_msg.angular.z > 0.25 ) { p3at_vel_msg.angular.z = 0.25; }
        if ( p3at_vel_msg.angular.z > 0.0 && std::abs(p3at_vel_msg.linear.x) < p3at_vel_msg.angular.z / 2 )
        {
            p3at_vel_msg.linear.x = - p3at_vel_msg.angular.z / 2;
        }
        ...
        if (accion_ard2 == ARD2_ATERRIZANDO)
        {
            if (!aproximacion_prima && !aproximacion_final)
            {
                longitud_objetivo_msg.data = (int)((altura_vuelo - altura_plataforma)*1000);
            }
            else if (aproximacion_prima && !aproximacion_final)
            {
                longitud_objetivo_msg.data = (int)(500);
            }
            else if (aproximacion_prima && aproximacion_final)
            {
                longitud_objetivo_msg.data = -500;
            }
        }
        else if (accion_ard2 == ARD2_ENTIERRA)
        {
            longitud_objetivo_msg.data = long_cable_encoder;
        }
        else if (accion_ard2 != EMERGENCIA && accion_p3at != EMERGENCIA)
        {
            if (fin_despegue)
            {
                longitud_objetivo_float = k_extension_cable * distancia_entre_robots * 1000;
                longitud_objetivo_msg.data = (int)longitud_objetivo_float;
                if (longitud_objetivo_msg.data < long_cable_min * 1000)
                {
                    longitud_objetivo_float = long_cable_min * 1000;
                    longitud_objetivo_msg.data = (int)longitud_objetivo_float;
                }
                else if (longitud_objetivo_msg.data > long_cable_max * 1000)
                {
                    longitud_objetivo_float = long_cable_max * 1000;
                    longitud_objetivo_msg.data = (int)longitud_objetivo_float;
                }
            }
            else
            {
                longitud_objetivo_float = long_cable_inic * 1000;
                longitud_objetivo_msg.data = (int)longitud_objetivo_float;
            }
        }
        ...
        //longitud_cable_pub.publish(longitud_objetivo_msg);

        if ((accion_ard2 != EMERGENCIA && accion_p3at != EMERGENCIA) || teleop_cable == true)
        {
            longitud_cable_pub.publish(longitud_objetivo_msg);
        }
