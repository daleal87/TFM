...
void PublishOdometry(ardrone_autonomy::Navdata navdata_raw)
{
  if (last_receive_time.isValid())
  {
    ...
    odometry[0] += ((cos((navdata_raw.rotZ / 180.0) * M_PI) *
                     navdata_raw.vx - sin((navdata_raw.rotZ / 180.0) * M_PI) *
                     -navdata_raw.vy) * delta_t) / 1000.0;
    odometry[1] += ((sin((navdata_raw.rotZ / 180.0) * M_PI) *
                     navdata_raw.vx + cos((navdata_raw.rotZ / 180.0) * M_PI) *
                     -navdata_raw.vy) * delta_t) / 1000.0;
  }
  ...
  odo_msg.pose.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(
        navdata_raw.rotX / 180.0 * M_PI,
        -navdata_raw.rotY / 180.0 * M_PI,
        -navdata_raw.rotZ / 180.0 * M_PI);
