...
tf::Vector3 ajuste_vel_ard2_aterrizaje;
...
void sonar_topic_Callback(const std_msgs::Int32::ConstPtr& ard2_kalman_sonar_msg)
{
    h_kalman_sonar = ard2_kalman_sonar_msg->data / 1000; // Altura segun el filtro de kalman con solo el sonar
}
...
{   
    if (h_kalman_odom * 0.6 > h_kalman_sonar && h_kalman_odom - h_kalman_sonar > 0.5 && accion_ard2 >= 2) {
...
geometry_msgs::Twist modo_seguimiento_p3at(double distancia_pto_obj, double desvio_orient_obj, int accion)
{
    ...
    if (accion == P3AT_SEGUIMIENTO)
    {
        ...
        {
            vel_msg.linear.x = control_velocidad_lineal(distancia_pto_obj, pre_p3at_error_x, kp_p3at, kd_p3at, ki_p3at,  p3at_vmax_linear);
            vel_msg.angular.z = control_velocidad_angular(desvio_orient_obj, 1, p3at_vmax_angular);
...
geometry_msgs::Twist control_p3at(geometry_msgs::Pose p_p3at, geometry_msgs::Pose p_obj_p3at)
{
    ...
    if (std::abs(desviacion_orientacion) < M_PI/4 ) {
...
double yaw_robot(geometry_msgs::Pose pose_robot)
{
    tf::Quaternion q(pose_robot.orientation.x, pose_robot.orientation.y, pose_robot.orientation.z,
                     pose_robot.orientation.w);
    return tf::getYaw(q);
...
geometry_msgs::PoseArray generar_trayectoria_esquiva(geometry_msgs::Pose pose, geometry_msgs::Pose pose_final, int n, double yaw_drone)
{
    ...
    if (n > 1)
    {
        ...
        {           
            ...
            temp_pose.position.x =
                    pose.position.x + (dif_x * i / n) * std::abs(cos(orientacion_cable + yaw_drone + incre_ang * (i+1)));
            temp_pose.position.y =
                    pose.position.y + (dif_y * i / n) * std::abs(sin(orientacion_cable + yaw_drone + incre_ang * (i+1)));
            temp_pose.position.z = 0.0;

            //temp_pose.orientation = tf::createQuaternionMsgFromYaw(orientacion_cable + yaw_drone + incre_ang * (i+2) - M_PI/2);

            temp_pose.orientation = tf::createQuaternionMsgFromYaw(atan2(
                        pose.position.y + (dif_y * (i+1) / n) * sin(orientacion_cable + yaw_drone + incre_ang * (i+2)) - temp_pose.position.y,
                        pose.position.x + (dif_x * (i+1) / n) * cos(orientacion_cable + yaw_drone + incre_ang * (i+2)) - temp_pose.position.x));
...
tf::Vector3 vel_p3at_en_base_ard2(geometry_msgs::Pose p_ard2, geometry_msgs::Pose p_p3at, geometry_msgs::Twist vel_p3at)
{
    tf::Transform tf_ard2, tf_p3at;
    ...
    tf::Vector3 v_p3at(vel_p3at.linear.x, vel_p3at.linear.y, 0.0);

    tf::Vector3 vel_p3at_B_ard2 = transf_mat_p3at.operator ()(v_p3at);

    return vel_p3at_B_ard2;
}
...
int main(int argc, char **argv)
{
    ...
    double lado_plataforma = 1; // Medida del lado de la plataforma. Nota: ahora es un cuadrado
    ...
    // Nombre topics
    nh_param.param<std::string>("ard2_pose_topic_name", ard2_pose_topic_name, "ground_truth/state");
    nh_param.param<std::string>("sonar_topic_name", sonar_topic_name, "ardrone/kalman_sonar");
    nh_param.param<std::string>("p3at_pose_topic_name", p3at_pose_topic_name, "p3at/kalman_odom");
    ...
    ros::Subscriber distancia_entre_robots_subs = node.subscribe("/cable/distancia_robots", 1, distrobotsCallback);
    ...
    while (node.ok()){
        ...
        if (activar_deteccion_obstaculos && accion_ard2 == ARD2_VUELOSOBRESUELO)
        {
            ...
            else if (deteccion_actual == deteccion_anterior && cuenta_obstaculo >= 1 && cuenta_obstaculo < 10)
            {
                cuenta_obstaculo++;
            }
            else if (deteccion_actual == deteccion_anterior && cuenta_obstaculo >= 10)
            {
        ...
        dist_centro_plataforma = lado_plataforma;
        ...
        else if (obstaculo_detectado && accion_ard2 == ARD2_VUELOSOBRESUELO)
        {
            ...
            yaw_ard2 = yaw_robot(pose_ard2);

            // Punto final tras esquivar el obstaculo
            pose_obj_p3at.position.x = pose_ard2.position.x + distancia_esquiva * std::abs(cos(orientacion_cable + (M_PI/2) + yaw_ard2));
            pose_obj_p3at.position.y = pose_ard2.position.y + distancia_esquiva * std::abs(sin(orientacion_cable + (M_PI/2) + yaw_ard2));
            ...
            trayectoria_esquiva = generar_trayectoria_esquiva(pose_p3at, pose_obj_p3at, n_ptos_esquiva, yaw_ard2);
        ...
        else if (accion_ard2 == ARD2_ATERRIZANDO)
        {
            ...
            {
                objetivo_aterrizaje.position.z = altura_plataforma + 0.5;
                //ard2_vel_msg = parar_robot();
            ...
            {
                objetivo_aterrizaje.position.z = altura_plataforma + 0.1;
                //ard2_vel_msg = parar_robot();
                //ard2_vel_msg.linear.z = 0.1;
            ...
            ajuste_vel_ard2_aterrizaje = vel_p3at_en_base_ard2(pose_ard2, pose_p3at, ard2_vel_msg);
            ard2_vel_msg.linear.x += ajuste_vel_ard2_aterrizaje.x();
            ard2_vel_msg.linear.y += ajuste_vel_ard2_aterrizaje.y();
        ...
        else if (accion_p3at == P3AT_ESQUIVAOBSTACULO)
        {
            if (cnt_esquiva < n_ptos_esquiva)
            {
                ...
                if (d_pto_esquiva_p3at < 0.3 /*&& rot_pto_esquiva_p3at < 0.3*/)
                {
                    cnt_esquiva++;
                    ROS_INFO("Pto esquiva: %i", cnt_esquiva);
                    ROS_INFO("Coordenadas - X: %2f / Y: %2f",
                ...
                /*else if (rot_pto_esquiva_p3at > M_PI/4)
                {
            ...
            else if (cnt_esquiva == n_ptos_esquiva)
            {
                p3at_vel_msg = parar_robot();
            }
        }
        ...
        else
        {
            if (teleop_p3at == true)
            {
                p3at_vel_msg = limitar_velocidad(joy_p3at_vel_msg, p3at_vmax_linear, p3at_vmax_angular);
            }
            else
            {
