...
std_msgs::Int32 accion_ard2_msg, accion_p3at_msg, velocidad_cable_msg, joy_cable_vel_msg;
...
double proyeccion_xy, orientacion_cable, inclinacion_cable, longitud_cable; // Informacion sobre el cable
...
{
    longitud_cable = msg->data; // Leemos el valor del topic que indica la longitud del cable
    if (longitud_cable > long_cable_max && activar_parada_longitud && accion_p3at != P3AT_ESPERAPIONNER && accion_ard2 != ARD2_APIONEER) // Comprobamos que no supere la distancia maxima permitida
    {
...
double control_velocidad_linear(double diferencia, double pre_error, double kp, double ki, double kd, double vmax)
{
...
geometry_msgs::Twist control_ard2(geometry_msgs::Pose p_ard2, geometry_msgs::Pose p_obj_ard2)
{
    ...
    vel_msg.linear.x = control_velocidad_linear(diff.getOrigin().x(), pre_ard2_error_x, kp_ard2, kd_ard2, ki_ard2,  ard2_vmax_linear);
    vel_msg.linear.y = control_velocidad_linear(diff.getOrigin().y(), pre_ard2_error_y, kp_ard2, kd_ard2, ki_ard2,  ard2_vmax_linear);
    vel_msg.linear.z = control_velocidad_linear(diff.getOrigin().z(), pre_ard2_error_z, kp_ard2, kd_ard2, ki_ard2,  ard2_vmax_linear);
...
geometry_msgs::Twist modo_seguimiento_p3at(double distancia_pto_obj, double desvio_orient_obj, int accion)
{
    ...
    if (accion == P3AT_SEGUIMIENTO)
    {
        ...
        {
            vel_msg.linear.x = control_velocidad_linear(distancia_pto_obj, pre_p3at_error_x, kp_p3at, kd_p3at, ki_p3at,  p3at_vmax_linear);
...
geometry_msgs::Twist control_p3at(geometry_msgs::Pose p_p3at, geometry_msgs::Pose p_obj_p3at)
{
    ...
    if (std::abs(desviacion_orientacion) < 0.2 ) {
        vel_msg.linear.x = control_velocidad_linear(distancia_objetivo, pre_p3at_error_x, kp_p3at, kd_p3at, ki_p3at,  p3at_vmax_linear);
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher velocidad_cable_pub = node.advertise<std_msgs::Int32>("guiado/velocidad_cable", 1);
    ...
    while (node.ok()){
        ...
        else if (accion_ard2 == ARD2_ATERRIZANDO)
        {
            ...
            {
                objetivo_aterrizaje.position.z = altura_plataforma + 0.4;
        ...
        {
            velocidad_cable_msg = joy_cable_vel_msg;
        }
        else
        {
            velocidad_cable_msg.data = control_cable(orientacion_cable, inclinacion_cable, pose_ard2, pose_p3at,
                                                     ard2_vel_msg, p3at_vel_msg);

            if (velocidad_cable_msg.data > 0)
            {
                velocidad_cable_msg.data = k_extension_cable * velocidad_cable_msg.data;
            }

            else
            {
                velocidad_cable_msg.data = k_recogida_cable * velocidad_cable_msg.data;
            }
        }
        ...
        velocidad_cable_pub.publish(velocidad_cable_msg);
