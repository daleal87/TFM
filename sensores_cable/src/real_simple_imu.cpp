...
std::string drone_frame_base, pioneer_frame_base, drone_frame_from_cable, drone_frame_full, pioneer_frame_full;
std_msgs::Int32 navdata_altd_msg;

void imucableCallback(const geometry_msgs::Vector3Stamped::ConstPtr& msg) // TO_DO: tipo mensaje
{
    orientacion = atan2(cos(msg->vector.y + M_PI/2), cos(msg->vector.x));
    inclinacion = std::min(sin((msg->vector.y + M_PI/2)), sin(msg->vector.x));
    inclinacion = asin(inclinacion);
...
{  
  ...
  //os::Publisher cable_drone_pos_pub = node.advertise<nav_msgs::Odometry>("/cable/drone_pos", 1);
  ros::Publisher navdata_altd_pub = node.advertise<std_msgs::Int32>("/ardrone/navdata_altitude", 1);

  ros::Timer timer_correccion = node.createTimer(ros::Duration(timer_trigger), correcionCallback);

  ros::NodeHandle nh_param("~");
  nh_param.param<double>("k_correccion_orient", k_correccion_orient, 0.0);
  ...
  nh_param.param<std::string>("drone_frame_from_cable", drone_frame_from_cable, "ardrone_base_from_cable");
  ...
  while (node.ok()){
    ...
    orientacion_cable_msg.data = orientacion;
    inclinacion_cable_msg.data = inclinacion;
    //proyeccionXY_cable_msg.data = longitud_cable * cos(inclinacion);
    proyeccionXY_cable_msg.data = dist_robots_msg.data  * cos(inclinacion);
    ...
    odo_msg.header.frame_id = drone_frame_base;
    odo_msg.child_frame_id = drone_frame_from_cable;
