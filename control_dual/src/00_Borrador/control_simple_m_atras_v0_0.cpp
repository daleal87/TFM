...
geometry_msgs::Twist control_p3at(geometry_msgs::Pose p_p3at, geometry_msgs::Pose p_obj_p3at)
{
    ...
    double desviacion_orientacion = M_PI - atan2(diff.getOrigin().y(), diff.getOrigin().x()); // Calculamos la desviacion con respcto al negativo del eje x
    if (desviacion_orientacion > M_PI) { desviacion_orientacion -= 2 * M_PI; } // Ajustamos el valor de la desviacion entre pi y -pi
...
geometry_msgs::Twist orientar_p3at(geometry_msgs::Pose p_p3at, geometry_msgs::Pose p_obj_p3at)
{
    ...
    vel_msg.linear.x = 0.0;
    vel_msg.angular.z = M_PI - tf::getYaw(diff.getRotation()); // Calculamos la desviacion con respeto al negativo del eje x
    if (vel_msg.angular.z > M_PI) { vel_msg.angular.z -= 2 * M_PI; } // Ajustamos el valor entre -pi y pi
    ...
    while (node.ok()){
        ...
        else
        {
            if (teleop_p3at == true)
            {
                ...
                if (p3at_vel_msg.linear.x > 0 ) { p3at_vel_msg.linear.x = 0; }
            }
            else
            {
        ...
        // TO_DO: implementar funcion que evite que las ruedas derechas vayan hacia atras
        //      - Investigar como funciona setvel() del paquete rosaria. La solucion deberia basarse en la inversa de dicha formula
