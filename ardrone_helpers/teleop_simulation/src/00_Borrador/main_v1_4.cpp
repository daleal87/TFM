...
struct TeleopArDrone
{
    ...
    ros::Publisher pub_takeoff, pub_land, pub_toggle_state, pub_vel, pub_vel_p3at, pub_modo_automatico, pub_modo_aterrizaje;
    ...
    std_msgs::Bool modo_automatico_msg, modo_aterrizaje_msg;
    ...
    void joyCb(const sensor_msgs::JoyConstPtr joy_msg){
        ...
        // Boton 4: iniciamos maniobra de aterrizaje (solo en modo manual)
        ...
        if (modo_automatico == false)
        {
            ...
            else if (boton_control_p3at && !dead_man_pressed && !control_p3at && !modo_aterrizaje){
                ROS_INFO("R2 pulsado. Controlando el p3at");
                control_ard2 = false;
                control_p3at = true;
            }
            else if (!boton_control_p3at && control_p3at && !modo_aterrizaje){
        ...
        else
        {
            ...
            if (is_flying && dead_man_pressed && activar_aterrizaje == true && !modo_aterrizaje)
            {
                modo_aterrizaje = true;
                modo_aterrizaje_msg.data = modo_aterrizaje;
                pub_modo_aterrizaje.publish(modo_aterrizaje_msg);
