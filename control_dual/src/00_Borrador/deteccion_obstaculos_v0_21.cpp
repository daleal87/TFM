...
double ard2_vmax_angular = 0.6;
double p3at_vmax_linear = 0.2;
double p3at_vmax_angular = 0.4;
...
{
    double vel = kp * diferencia * abs(diferencia);
...
geometry_msgs::Twist control_p3at(geometry_msgs::Pose p_p3at, geometry_msgs::Pose p_obj_p3at)
{
    ...
    if (desviacion_orientacion < 0.2 && desviacion_orientacion > -0.2) {
...
int main(int argc, char **argv)
{
    ...
    while (node.ok()){
        ...
        else if (accion_ard2 == ARD2_MARCAJEOBSTACULO && accion_p3at == P3AT_ESQUIVAOBSTACULO)
        {
            ...
            if (en_pto == true && rotacion_a_objetivo < 0.2)
            {
            ...
            if (modo_automatico == true)
            {
                if (fin_despegue == false)
                {
                    ...
                    if (distancia_a_objetivo < 0.3 && distancia_a_altura < 0.2)
                    {
                        ...
                        ROS_INFO("Objetivo X: %2f", objetivo_mision.position.x); // DEBUG
                        ROS_INFO("Objetivo Y: %2f", objetivo_mision.position.y); // DEBUG
                        ROS_INFO("Objetivo Z: %2f", objetivo_mision.position.z); // DEBUG
                    }
                }
                else if (fin_despegue == true && fin_mision == false)
                {
                    ...
                    ROS_INFO("Distancia a objetivo: %2f", distancia_a_objetivo); // DEBUG
        ...
        if (accion_p3at == P3AT_ESQUIVAOBSTACULO)
        {
            ...
            else if (en_pto == true && rotacion_a_objetivo > 0.2)
            {
