...
int main(int argc, char **argv)
{
    ...
    //ros::Subscriber kalman_ard2_odom_subs = node.subscribe("ardrone/kalman_odom", 1, ard2_pose_topic_Callback);
    ...
    while (node.ok()){
        ...
        if (accion_ard2 == ARD2_ATERRIZANDO && aproximacion_prima)
        {
            longitud_objetivo_msg.data = (int)((altura_vuelo - altura_plataforma) * k_recogida_cable * 1000);
        }
        ...
        else if (accion_ard2 != EMERGENCIA || accion_p3at != EMERGENCIA)
        {
            ...
            longitud_objetivo_msg.data = (int)longitud_objetivo_float;
            if (longitud_objetivo_msg.data < long_cable_min)
            {
