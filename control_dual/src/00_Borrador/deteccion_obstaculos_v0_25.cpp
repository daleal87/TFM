...
void modoaterrizajeCallback(const std_msgs::Bool::ConstPtr& msg)
{
    if (accion_ard2 != ARD2_ATERRIZANDO && msg->data == true)
    {
        ...
        // accion_p3at = P3AT_ESPERAPIONNER; // TO_DO: aterrizaje++
        // p3at_vel_msg = parar_robot(); // TO_DO: aterrizaje++
...
int main(int argc, char **argv)
{
    ...
    while (node.ok()){
        ...
        // CONDICION 0 || ~~ PDTE ~~  // TO_DO: probar que funciona bien
        if (modo_emergencia)
        {
        ...
        // CONDICION 6 || PDTE: probar que funciona
