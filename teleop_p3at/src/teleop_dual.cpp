...
class TeleopRosAria
{
  ...
  private:
    ...
    ros::Publisher twist_pub_r, twist_pub_g;
};
  ...
  step_angular_(0.1)
{
  ...
  twist_pub_r = nh_.advertise<geometry_msgs::Twist>("RosAria/cmd_vel", 1);
  twist_pub_g = nh_.advertise<geometry_msgs::Twist>("sim_p3at/cmd_vel", 1);
}
int kfd = 0;
...
int main(int argc, char** argv)
{
  ...
  ros::init(argc, argv, "teleop_gazebo");
...
void TeleopRosAria::keyLoop()
{
  ...
  for(;;)
  {
    ...
    if(dirty ==true)
  	{
  	  twist_pub_r.publish(twist);
          twist_pub_g.publish(twist);
  	  dirty=false;
  	}
  }
  return;
