...
geometry_msgs::Twist control_ard2(geometry_msgs::Pose p_ard2, geometry_msgs::Pose p_obj_ard2)
{
    ...
    vel_msg.linear.x = kp * (p_obj_ard2.position.x - p_ard2.position.x);
    vel_msg.linear.y = kp * (p_obj_ard2.position.y - p_ard2.position.y);
    vel_msg.linear.z = kp * (p_obj_ard2.position.z - p_ard2.position.z);

    tf::Quaternion q_obj(
        p_obj_ard2.orientation.x,
        p_obj_ard2.orientation.y,
        p_obj_ard2.orientation.z,
        p_obj_ard2.orientation.w);

    tf::Quaternion q(
        p_ard2.orientation.x,
        p_ard2.orientation.y,
        p_ard2.orientation.z,
    ...
    vel_msg.angular.z = kp * (yaw_obj - yaw);
