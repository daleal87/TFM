...
geometry_msgs::Twist control_ard2(geometry_msgs::Pose p_ard2, geometry_msgs::Pose p_obj_ard2)
{
    ...
    vel_msg.linear.x = diff.getOrigin().x();
    vel_msg.linear.y = diff.getOrigin().y();
    vel_msg.linear.z = diff.getOrigin().z();
    vel_msg.angular.z = tf::getYaw(diff.getRotation());

    // tf_ruta = tf_ard2.inverse() * tf_obj_ard2;

    // v_ruta = tf_ruta.getOrigin();
    // q_ruta = tf_ruta.getRotation();

    /*vel_msg.linear.x = kp * v_ruta.x();
    vel_msg.linear.y = kp * v_ruta.y();
    vel_msg.linear.z = kp * v_ruta.z();*/

    // vel_msg.linear.x = kp * tf_ruta.getOrigin().x();
    // vel_msg.linear.y = kp * tf_ruta.getOrigin().y();
    // vel_msg.linear.z = kp * tf_ruta.getOrigin().z();


    // double roll_ruta, pitch_ruta, yaw_ruta;
    // tf::Matrix3x3(q_ruta).getRPY(roll_ruta, pitch_ruta, yaw_ruta);

    // double roll_obj, pitch_obj, yaw_obj;
    // double roll, pitch, yaw;

    // tf::Matrix3x3(q_obj).getRPY(roll_obj, pitch_obj, yaw_obj);
    // tf::Matrix3x3(q).getRPY(roll, pitch, yaw);

    //vel_msg.angular.z =  kp * yaw_ruta;
...
int main(int argc, char **argv)
{
    ...
    while (node.ok()){
          ...
          else if (accion_ard2 == ARD2_MARCAJEOBSTACULO && accion_p3at == P3AT_ESQUIVAOBSTACULO)
          {
              ...
              accion_p3at = P3AT_ESPERAPIONNER; // TO_DO: Eliminar, solo para prueba
          ...
          // CONDICION 5 ||  ~~ PDTE ~~
          else if (accion_ard2 == ARD2_MARCAJEOBSTACULO && accion_p3at == P3AT_ESPERAPIONNER && proyeccion_xy > dist_centro_plataforma)
          {
              if (accion_ard2 != ARD2_APIONEER) { accion_ard2 = ARD2_APIONEER; }
