...
double pre_ard2_error_x, pre_ard2_error_y, pre_ard2_error_z, pre_p3at_error_x, pre_p3at_error_y, pre_p3at_error_z;
...
void longitudcableCallback(const std_msgs::Float64::ConstPtr& msg)
{
    ...
    if (longitud_cable > long_cable_max && accion_p3at != P3AT_ESPERAPIONNER && accion_ard2 != ARD2_APIONEER) // Comprobamos que no supere la distancia maxima permitida
    {
...
int control_cable(tf::Quaternion q_cab, geometry_msgs::Pose p_ard2, geometry_msgs::Pose p_p3at,
                  geometry_msgs::Twist vel_ard2, geometry_msgs::Twist vel_p3at)
{
    double v_cable_x = 0.0;
    int v_cable = 0;

    tf::Transform tf_ard2, tf_p3at, tf_cable, tf_v_ard2, tf_v_p3at;

    tf::Vector3 pos_cable(p_p3at.position.x, p_p3at.position.y, p_p3at.position.z);

    tf::Vector3 v_ard2(vel_ard2.linear.x, vel_ard2.linear.y, vel_ard2.linear.z);
    tf::Vector3 v_p3at(vel_p3at.linear.x, vel_p3at.linear.y, vel_p3at.linear.z);

    //tf_cable.setOrigin(pos_cable);
    tf_cable.setRotation(q_cab);

    tf::Vector3 diff_ard2 = tf_cable.operator ()(v_ard2);
    tf::Vector3 diff_p3at = tf_cable.operator ()(v_p3at);

    v_cable_x = (diff_ard2[0] + diff_p3at[0]) * 1000; // Velocidad del cable en mm
    v_cable = (int)v_cable_x;
    ...
    tf::Pose diff_ard2 = tf_cable.inverse() * tf_ard2; // Transformamos la pose del ard2 a la base del cable
    tf::Pose diff_p3at = tf_cable.inverse() * tf_p3at; // Transformamos la pose del p3at a la base del cable

    // Transformamos las velocidades a la base del cable
    tf::Vector3 v_ard2(vel_ard2.linear.x, vel_ard2.linear.y, vel_ard2.linear.z); // Vector de velocidad del ard2 en la base del ard2
    tf::Vector3 v_p3at(vel_p3at.linear.x, vel_p3at.linear.y, vel_p3at.linear.z); // Vector de velocidad del ard2 en la base del ard2

    tf_v_ard2.setOrigin(v_ard2);
    tf_v_ard2.setRotation(q_ard2);

    tf_v_p3at.setOrigin(v_p3at);
    tf_v_p3at.setRotation(q_p3at);

    tf::Pose diff_v_ard2 = diff_ard2.inverse() * tf_ard2; // Transformamos la velocidad del ard2 a la base del cable
    tf::Pose diff_v_p3at = diff_p3at.inverse() * tf_p3at; // Transformamos la velocidad del p3at a la base del cable

    v_cable_x = (diff_v_ard2.getOrigin().x() - diff_v_p3at.getOrigin().x()) * 1000; // Velocidad del cable en mm
    v_cable = (int)v_cable_x;
    ...
    */
    ...
    /*
    tf::Transform tf_ard2, tf_p3at, tf_cable;

    tf::Vector3 p_cable(p_p3at.position.x, p_p3at.position.y, p_p3at.position.z);
    ...
    tf_ard2.setOrigin(v_ard2);
    tf_ard2.setRotation(q_ard2);

    tf_p3at.setOrigin(v_p3at);
    tf_p3at.setRotation(q_p3at);

    tf_cable.setOrigin(p_cable);
    tf_cable.setRotation(q_cab);

    tf::Pose diff_ard2 = tf_cable.inverse() * tf_ard2; // Transformamos las velocidades a la base del cable
    tf::Pose diff_p3at = tf_cable.inverse() * tf_p3at; // Transformamos las velocidades a la base del cable

    v_cable_x = (diff_ard2.getOrigin().x() - diff_p3at.getOrigin().x()) * 1000; // Velocidad del cable en mm
    v_cable = (int)v_cable_x;
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber longitud_cable_subs = node.subscribe("/cable/longitud_gps", 1, longitudcableCallback);
    ...
    while (node.ok()){
        ...
        {
            velocidad_cable_msg.data = control_cable(q_cable, pose_ard2, pose_p3at, ard2_vel_msg, p3at_vel_msg);
        }
