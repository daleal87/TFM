...
void sonar_topic_Callback(const nav_msgs::Odometry::ConstPtr& ard2_kalman_sonar_msg)
{
...
{    
    ...
    {
        teleop_cable = true;
        longitud_objetivo_float += vel_msg->data * dt;
...
geometry_msgs::Twist modo_seguimiento_p3at(double distancia_pto_obj, double desvio_orient_obj, int accion)
{
    ...
    if (accion == P3AT_SEGUIMIENTO)
    {
        ...
        if (distancia_pto_obj > distancia_a_ard2 && std::abs(diff_orientaciones) < M_PI/4 )
        {
            ...
            //vel_msg.angular.z = control_velocidad_angular_p3at(desvio_orient_obj, 1, p3at_vmax_angular);

            vel_msg.angular.z = kp * diferencia;
            ...
            {
                if ( (2*M_PI-diferencia)/vmax < diferencia/0.25 ) { vel_msg.angular.z = -vmax; }
                else { vel = 0.25; }
            }
            else if (vel_msg.angular.z < 0  && vel_msg.angular.z < -vmax) { vel_msg.angular.z = -vmax; }
        }
        else if ( distancia_pto_obj > distancia_a_ard2 && std::abs(diff_orientaciones) >= M_PI/4 )
        {
            vel_msg.linear.x = - control_velocidad_lineal(distancia_pto_obj, pre_p3at_error_x, kp_p3at, kd_p3at, ki_p3at,  p3at_vmax_linear); // m_atras
            //vel_msg.angular.z = control_velocidad_angular_p3at(desvio_orient_obj, 1, p3at_vmax_angular);
...
int main(int argc, char **argv)
{
    ...
    nh_param.param<double>("long_cable_min", long_cable_min, 1);
    ...
    while (node.ok()){
        ...
        else if (accion_ard2 == ARD2_ATERRIZANDO)
        {
            ...
            if (!aproximacion_prima)
            {
                ...
                if (h_kalman_sonar > (altura_vuelo - altura_plataforma)*1000)
                {
            ...
            else if (aproximacion_prima && !aproximacion_final)
            {
                ...
                if (long_cable_encoder < 300 && h_kalman_sonar < 0.2)
                {
        ...
        else if (accion_p3at == P3AT_AFINDEMISION)
        {
            ...
            rot_objetivo_p3at = std::abs(desviacion_yaw(pose_p3at, pose_obj_p3at)); // m_atras
        ...
        if (accion_ard2 == ARD2_ATERRIZANDO && !aproximacion_final)
        {
            longitud_objetivo_msg.data = (int)(long_cable_min * k_recogida_cable * 1000);
        }
        else if (accion_ard2 == ARD2_ATERRIZANDO && aproximacion_final)
        {
            longitud_objetivo_msg.data = 200;
        }
