...
std::string ard2_pose_topic_name, p3at_pose_topic_name;
...
int main(int argc, char **argv)
{
    ...
    // Subcribers
    ros::Subscriber proyeccionXY_cable_subs = node.subscribe("cable/proyeccionXY_real", 1, proyeccionXYCallback);
    ros::Subscriber orientacion_cable_subs = node.subscribe("cable/orientacion_real", 1, orientacionCallback);
    ros::Subscriber inclinacion_cable_subs = node.subscribe("/cable/inclinacion_real", 1, inclinacionCallback);
    ros::Subscriber longitud_cable_subs = node.subscribe("/cable/longitud_real", 1, longitudcableCallback);
    ...
    ros::Subscriber joy_ard2_vel_subs =  node.subscribe("guiado/joy_a/p3at/odomrd2_vel", 1, joyard2velCallback);
    ...
    ros::Publisher ard2_vel_pub = node.advertise<geometry_msgs::Twist>("cmd_vel", 5);
    ros::Publisher p3at_vel_pub = node.advertise<geometry_msgs::Twist>("p3at/cmd_vel", 5);
